...
function urlForInline(self, url, field_action) {
  var inline_form = self.closest('.form.element-editor-editform__form');
  if (inline_form.length > 0) {
    var base_action = inline_form.attr('action');
    var link_name = self.siblings('input.link').attr('name');
    if (!link_name) {
...
function updateElements(self) {
  var inline_form = self.closest('.form.element-editor-editform__form');
  if (inline_form.length > 0) {
    var link_input = self.siblings('input.link');
    if (link_input.length == 0) {
      link_input = self.closest('input.link');
    }
    var link_value = link_input.val();
    var link_name = link_input.attr('name');
    var form_name = inline_form.attr('id').replace('Form_', '');
    var elements = (0, _jquery2.default)('input[type=hidden][value*="ElementForm"].no-change-track');

    var data = JSON.parse(elements.val());
...
var _Injector = __webpack_require__(16);

var _Injector2 = _interopRequireDefault(_Injector);

var _linkfield = __webpack_require__(10);
...
var LinkField = function (_React$PureComponent) {
    _inherits(LinkField, _React$PureComponent);

    function LinkField(props) {
        _classCallCheck(this, LinkField);

        return _possibleConstructorReturn(this, (LinkField.__proto__ || Object.getPrototypeOf(LinkField)).call(this, props));
    }
    ...
    _createClass(LinkField, [{
        ...
        value: function render() {
            var _props = this.props,
                ...
                value = _props.value,
                url = _props.url,
                holderId = _props.holderId;


            var have_link = value != undefined && value != '' && value != '0';

            var link = React.createElement(
                'a',
                { className: 'link', href: url, style: { paddingRight: '10px' } },
                label
            );
            var add = React.createElement(
                'button',
                { href: '#', className: 'linkfield-button btn btn-primary font-icon-plus-circled' },
                'Add Link'
            );
            var edit = React.createElement(
                'button',
                { href: '#', className: 'linkfield-button btn btn-primary' },
                'Edit'
            );
            var remove = React.createElement(
                'button',
                { href: '#', className: 'linkfield-remove-button btn btn-danger' },
                'Remove'
            );

            return React.createElement(
                'div',
                { className: 'field form-group link', id: holderId },
                React.createElement(
                    'label',
                    { className: 'form__field-label', id: 'title-' + id, htmlFor: id },
                    title
                ),
                React.createElement(
                    'div',
                    { className: 'form__field-holder' },
                    React.createElement('div', { className: 'linkfield-dialog' }),
                    React.createElement('input', { type: 'hidden', className: 'link', id: id, name: name, value: value }),
                    have_link && link,
                    have_link && edit,
                    have_link && remove,
                    !have_link && add
